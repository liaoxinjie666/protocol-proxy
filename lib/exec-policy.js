...
class ExecPolicy {
  ...
  check(command) {
    ...
    // 评估顺序：User forbidden > Default forbidden > User allow > Default allow > User prompt > Default prompt
    // 1. forbidden（deny 优先）
   ...
   * 永久添加 allow 规则到用户配置
   */
  addAllowRule(pattern, description) {
    // 避免重复
    if (this.userRules.allow.some(r => r.pattern === pattern)) return;
    this.userRules.allow.push({ pattern, description: description || `用户允许: ${pattern}` });
    this.save();
  }

  /**
   * 永久添加 forbidden 规则到用户配置
   */
  addForbiddenRule(pattern, description) {
    if (this.userRules.forbidden.some(r => r.pattern === pattern)) return;
    this.userRules.forbidden.push({ pattern, description: description || `用户禁止: ${pattern}` });
    this.save();
  }
   ...
   * 永久添加 prompt 规则到用户配置
   */
  addPromptRule(pattern, description) {
    if (this.userRules.prompt.some(r => r.pattern === pattern)) return;
    this.userRules.prompt.push({ pattern, description: description || `用户需确认: ${pattern}` });
    this.save();
  }
...
module.exports = { ExecPolicy, execPolicy, DEFAULT_RULES, POLICY_PATH };
