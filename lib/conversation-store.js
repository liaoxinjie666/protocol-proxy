...
function create(proxyId, maxConversations, chatMode) {
  // 超过最大会话数时删除最早的
  ...
  const conv = { id, proxyId, messages: [], mode: 'full', windowSize: 20, chatMode: chatMode || 'assistant', createdAt: Date.now(), lastActivity: Date.now() };
...
// @param {string} [filterChatMode] - 可选，按 chatMode 过滤 ('assistant'|'pure')
function list(filterChatMode) {
  return Object.values(conversations)
    .filter(c => !filterChatMode || (c.chatMode || 'assistant') === filterChatMode)
    .map(c => ({
      id: c.id,
      proxyId: c.proxyId,
      mode: c.mode || 'full',
      windowSize: c.windowSize || 20,
      chatMode: c.chatMode || 'assistant',
