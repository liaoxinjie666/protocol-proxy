...
let enabledSkills = new Set(); // 纯净模式下额外启用的非系统技能名称
...
function getAvailableForChat(isPure) {
  return Object.values(skills).filter(s => {
    if (!isPure) return true; // 助手模式：所有技能可用
    // 纯净模式：system 技能隐藏，preset/user 技能需在 enabledSkills 中
    if (s.category === 'system') return false;
    return enabledSkills.has(s.name);
  }).map(s => {
 ...
 * 从设置中恢复纯净模式启用的技能列表
 */
function loadEnabledSkills(settings) {
  enabledSkills = new Set();
  if (settings && settings.pureModeSkills) {
    const arr = Array.isArray(settings.pureModeSkills) ? settings.pureModeSkills : [];
    arr.forEach(name => enabledSkills.add(name));
  }
}
...
function setSkillEnabled(name, enabled) {
  if (enabled) enabledSkills.add(name);
 ...
 * 获取所有技能及其纯净模式启用状态
 */
function getSkillsWithStatus() {
  return Object.values(skills).map(s => ({
    name: s.name,
    description: s.description,
    category: s.category,
    pureModeEnabled: s.category === 'system' ? false : enabledSkills.has(s.name),
    isSystem: s.category === 'system',
  }));
}

/**
 * 获取当前纯净模式启用的技能名列表（用于持久化）
 */
...
module.exports = {
  init, list, get, create, createFromUpload, update, remove,
  getAvailableForChat, loadEnabledSkills, setSkillEnabled, getSkillsWithStatus, getEnabledSkillNames,
};
