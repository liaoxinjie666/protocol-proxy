...
function buildSystemPrompt({ skillStore, mcpClient, memoryManager, agentStore }) {
  ...
  const sections = [
    ...
    `execute_code 工具使用指引：\n- 当用户需要数据分析、画图表、文件处理、计算时，使用 execute_code\n- 推荐使用 Python（默认），简单处理可用 JavaScript\n- 画图表：用 matplotlib/seaborn，保存为 PNG 到当前目录（plt.savefig('output.png')），图片会自动返回\n- 用户上传的文件通过 files 参数传入，代码中直接用文件名打开（如 open('data.csv')）\n- 输出应简洁，避免打印大量数据（超长输出会被截断）\n- 处理完后给出结论性总结，不要复述代码`,
