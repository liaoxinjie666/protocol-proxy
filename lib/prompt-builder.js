...
/**
 * 构建技能段落
 * @param {Object} skillStore
 * @param {boolean} isPure - 是否纯净模式
 */
function buildSkillsSection(skillStore, isPure) {
  const skills = skillStore.getAvailableForChat(isPure);
 ...
 * @param {string} [opts.chatMode] - 'assistant'（助手模式，默认）或 'pure'（纯净模式）
 */
function buildSystemPrompt({ skillStore, mcpClient, memoryManager, agentStore, convFiles, multimodalToolNames, chatMode }) {
  const isPure = chatMode === 'pure';
  const now = new Date().toLocaleString('zh-CN', { hour12: false });
  const blocks = memoryManager?.getPromptBlocks() || {};

  // ---- 角色声明（两种模式不同） ----
  const roleDeclaration = isPure
    ? (blocks.soul
        ? `你是一个智能 AI 助手。你可以帮助用户进行对话、写作、分析、创意、编程等各种任务。你具备文件读写、代码执行、文档解析等通用能力。当前时间：${now}\n\n你的人设（优先遵循）：\n${blocks.soul}`
        : `你是一个智能 AI 助手。你可以帮助用户进行对话、写作、分析、创意、编程等各种任务。你具备文件读写、代码执行、文档解析等通用能力。当前时间：${now}`)
    : (blocks.soul
  ...
  // ---- 行为规则（两种模式不同） ----
  const behaviorRules = isPure
    ? `规则：\n- 当用户需要查看或修改文件、执行命令时，使用对应的文件和命令工具\n- 当用户需要数据分析、文件处理、画图表时，使用 execute_code 工具\n- 当用户只是打招呼、闲聊、或询问一般问题时，直接回答\n- 执行写操作或危险命令前，先告知用户将要做什么并确认`
  ...
  // ---- 职责（两种模式不同） ----
  const responsibilities = isPure
    ? `你的职责：\n1. 帮助用户完成各种对话、写作和分析任务\n2. 读写文件、执行代码、处理数据\n3. 解析文档，提取和处理内容\n4. 用自然语言解释技术问题`
  ...
  const toolGuidance = (() => {
    if (isPure) {
      const lines = [
        '- parse_document：解析文档文件（PDF/DOCX/PPTX/XLSX），快速提取文本和内嵌图片，比 execute_code 快。当用户上传了文档文件时，优先使用此工具',
        "- execute_code：执行 Python/JS 代码，用于数据分析、画图表、复杂处理、格式转换。图表应保存为 PNG（plt.savefig('output.png')），图片会自动返回",
        "- access_file：读取本地文件为多模态内容（模型可直接看到图片、听到音频）。重要规则：当用户提到或询问任何图片/音频/视频文件时（无论是生成的、上传的、本地已有的、还是对话中出现过的文件），你必须主动调用 access_file 加载该文件，不要说'我无法看到/听到'。用文件名或路径即可",
        "- 需要使用对话中的文件时，通过 execute_code 的 files 参数指定文件名（如 files: ['data.csv']），只传代码实际需要的文件",
        '- execute_code 输出应简洁，避免打印大量数据（超长输出会被截断），处理完后给出结论性总结',
      ];
      const mmNames = multimodalToolNames || [];
      if (mmNames.length > 0) {
        const mmLabel = { generate_image: '图片生成', generate_video: '视频生成', text_to_speech: '语音合成', generate_music: '音乐生成' };
        lines.push(`- ${mmNames.map(n => `${n}（${mmLabel[n] || n}）`).join(' / ')}：多模态生成工具，当用户需要生成图片、视频、语音、音乐时使用`);
      }
      return `工具使用指引：\n${lines.join('\n')}`;
    }
    // 助手模式：完整指引
  ...
  const sections = [
    ...
    // 多模态展示指引（仅在有多模态工具时注入）
    (multimodalToolNames && multimodalToolNames.length > 0)
      ? `多模态内容展示规则：\n当你需要在聊天中向用户展示图片、音频或视频时，必须使用以下专用标记格式（不要用 HTML 标签如 <audio> <video>，系统会过滤 HTML）：\n- 本地文件：[MEDIA:audio:文件名] 或 [MEDIA:video:文件名]\n- 公网URL：[MEDIA:video_url:完整URL]\n- 图片：[MEDIA:image:文件名]\n文件名从工具返回的 files 字段获取（数组），或从 audio_file / video_file 字段获取。\n使用示例：工具返回 {"files":["tts_123.wav"]}，则回复中写"音频：[MEDIA:audio:tts_123.wav]"，前端自动展示播放器。\n只使用工具返回的真实文件名或URL，不要自行编造。`
      : '',

    // 对话文件可用性
    convFiles && convFiles.length > 0
      ? `当前对话可用文件（在 execute_code 的 files 参数中指定需要的文件名）：\n${convFiles.map(f => `- ${f}`).join('\n')}`
      : '',
    ...
    // 技能列表（按模式过滤）
    buildSkillsSection(skillStore, isPure),

    // 代理身份列表（仅助手模式）
    isPure ? '' : buildAgentsSection(agentStore),

    // MCP 工具列表（仅助手模式）
    isPure ? '' : buildMcpSection(mcpClient),
