 ...
 */
function buildSystemPrompt({ skillStore, mcpClient, memoryManager, agentStore, convFiles, multimodalToolNames }) {
  ...
  const sections = [
    ...
    (() => {
      const lines = [
        ...
        "- 用户在对话中上传的文件会自动注入到代码工作目录，代码中直接用文件名打开（如 open('data.csv')），无需手动指定",
    ...
    convFiles && convFiles.length > 0
      ? `当前对话已上传以下文件，可通过 execute_code 直接使用（代码中用文件名打开即可）：\n${convFiles.map(f => `- ${f}`).join('\n')}`
      : '',
