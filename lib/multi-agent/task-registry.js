...
class TaskRegistry extends EventEmitter {
  ...
  create({ objective, scope, scopePath, model, toolsetNames, parentTaskId, dependencies, role, batchId }) {
    const task = {
  ...
  emitBatchCreated(batchId, tasks) {
    this.emit('batch:created', { batchId, tasks: tasks.map(t => ({ id: t.id, objective: t.objective, role: t.role })) });
  }
