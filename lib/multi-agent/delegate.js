 ...
 */
function buildChildSystemPrompt({ role, scopeDir, allowedTools, blockedSet, autoDenySet, roleBlocked, projectRoot }) {
...
async function delegateTask({
  ...
  const tasks = goals.map(goal => {
    const roleName = typeof goal === 'object' && goal.role ? goal.role : (config.role || 'general');
    return effectiveRegistry.create({
...
async function continueTask({
  ...
  // 获取角色和工具策略
  const role = getRole(task.role || 'general');
  const { blockedTools: roleBlocked, autoDenyTools: roleAutoDeny } = mergeToolPolicy(role, cfg.blockedTools, cfg.autoDenyTools);
  const taskBlockedSet = new Set(roleBlocked);
  const taskAutoDenySet = new Set(roleAutoDeny);
  const baseHandlers = filterToolHandlers(toolHandlers, taskBlockedSet);
  ...
  // 重新启动任务
  const abortController = new AbortController();
  registry.start(taskId, abortController);
