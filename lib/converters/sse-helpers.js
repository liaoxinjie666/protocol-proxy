...
// Anthropic 风格 SSE：event: <type>\ndata: <json>\n\n
// Responses API 的 SSE 同样使用这种带 event: 行的格式，故 responses 转换器也复用此函数。
function encodeAnthropicEvent(eventName, obj) {
  return `event: ${eventName}\ndata: ${JSON.stringify(obj)}\n\n`;
}

// Responses API 流终止符。OpenAI Responses API 以 response.completed 作为语义终止，
// 但客户端 SDK 通常仍依赖 data: [DONE] 判断传输层流结束。
function encodeDone() {
  return 'data: [DONE]\n\n';
}
