    ...
    if (state.isFreeform) {
      ...
      try {
        ...
        if (keys.length > 0 && typeof parsed[keys[0]] === 'string') {
          state.arguments = parsed[keys[0]];
          dbg(`[a2r] freeform unwrap: JSON keys=${JSON.stringify(keys)} → extracted first key="${keys[0]}" len=${state.arguments.length}`);
        } else {
          dbg(`[a2r] freeform unwrap: JSON parsed but no string value in first key. keys=${JSON.stringify(keys)} types=${JSON.stringify(keys.map(k => typeof parsed[k]))}`);
        }
      } catch (e) {
        dbg(`[a2r] freeform unwrap: not JSON (keeping raw). error=${e.message} raw=${rawArgs.slice(0, 100)}`);
      }
