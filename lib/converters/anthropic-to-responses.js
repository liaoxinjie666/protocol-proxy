...
function createSSEConverter(targetModel, freeformTools, namespaceMap) {
  ...
  function emitReasoningDone() {
    ...
    accumulatedReasoning = ''; // 清零：支持后续重新开启 reasoning item（多个 thinking 块场景）
    return result;
  }
  ...
  function emitTextDone() {
    ...
    // 兜底：检查累积文本里是否有 freeform XML 工具调用（模型偶发把工具调用写成 XML 文本）
    // 如果有，提取出来转成 function_call items，文本只保留清理后的部分
    ...
    if (xmlCalls.length > 0 && cleanedText.length === 0) {
      // 文本全是 XML 工具调用（无实际文本内容）→ 跳过 message item，不发 output_item.done
      // 从 outputItems 移除这个空 message item
      const idx = outputItems.findIndex(i => i.id === textItemId);
      if (idx >= 0) outputItems.splice(idx, 1);
      // 发 function_call items 替代
      result += emitXmlToolCalls(xmlCalls);
      return result;
    }

    // 有实际文本（可能含部分 XML），更新 message item 文本为清理后的内容
  ...
  // 把 freeform XML 提取的工具调用转成 function_call items 并发出完整事件序列
  function emitXmlToolCalls(calls) {
    ...
    for (const call of calls) {
      ...
      dbg('[a2r] freeform XML extracted: name=' + call.name + ' args=' + JSON.stringify(call.arguments).slice(0, 100));
    }
    return result;
  }
        ...
        switch (et) {
          ...
          case 'content_block_start': {
            ...
            if (cb?.type === 'tool_use') {
              ...
              // anthropic image 输出块：Responses API 无标准图片输出 item，用占位文本提示
              // （避免静默丢失；模型生成图片的输出场景较罕见）
              result += handleTextDelta('[模型输出了一张图片，当前协议不支持透传]');
            }
      ...
      if (buffer) {
        ...
        if (buffer.startsWith('data: ')) {
          try {
            ...
            // 提取 message_delta 里的 usage（convertChunk 可能因末尾无换行而漏掉）
            if (data.usage) {
              usageData = {
