...
function convertRequest(body, targetModel) {
  ...
  // 转换消息角色和内容（如果历史中有 reasoning_content，则所有 assistant 消息都需要 thinking 块）
  const hasReasoning = otherMessages.some(m => m.reasoning_content);
  result.messages = otherMessages.map(msg => convertMessage(msg, hasReasoning));
...
function makeThinkingBlock(reasoning) {
  const text = typeof reasoning === 'string' ? reasoning
    : Array.isArray(reasoning) ? reasoning.map(b => b.thinking || b.text || '').join('')
    : '';
  return { type: 'thinking', thinking: text || '(continued)' };
}

function convertMessage(msg, forceThinking) {
  if (msg.role === 'tool') {
  ...
  if (msg.role === 'assistant' && msg.tool_calls) {
    const content = [];
    if (needThinking) content.push(makeThinkingBlock(msg.reasoning_content));
    if (msg.content) {
  ...
  if (Array.isArray(msg.content)) {
    ...
    if (needThinking) {
      content.unshift(makeThinkingBlock(msg.reasoning_content));
    }
