...
function convertMessage(msg, forceThinking = false, reasoningCache = null) {
  ...
  // 获取 reasoning 内容：优先使用消息中的，否则从缓存获取
  // 缓存 key 必须与 proxy-server 的 getReasoningKey 一致：content + '|' + toolIds
  ...
  if (!reasoningContent && forceThinking && reasoningCache) {
    ...
    const cacheKey = msg.content + '|' + toolIds;
    reasoningContent = reasoningCache.get(cacheKey) || reasoningCache.get((msg.content?.slice(0, 100) || '') + '|');
  }
...
function convertResponse(anthropicBody) {
  ...
  // 提取文本内容、tool_calls、thinking 块和 image 块
  const toolCalls = [];
  const textParts = [];
  const thinkingParts = [];
  const imageParts = [];
  ...
  for (const block of (anthropicBody.content || [])) {
    if (block.type === 'text') {
      ...
      // anthropic image 块 → openai image_url（base64 转成 data URL）
  ...
  // 有图片时 content 用数组形式（text + image_url），否则用纯文本字符串
  if (imageParts.length > 0) {
    const contentArr = [];
    if (textParts.length > 0) contentArr.push({ type: 'text', text: textParts.join('') });
    contentArr.push(...imageParts);
    choice.message.content = contentArr;
  } else {
...
function processLine(line, state, targetModel) {
  ...
  switch (event.type) {
    ...
    case 'content_block_start': {
      ...
      } else if (state.blockType === 'image') {
        // anthropic image 输出块：openai chat 无标准图片输出，用占位文本提示
        state.pendingImagePlaceholder = true;
      }
      return '';
    }
    ...
    case 'content_block_stop': {
      // image 块结束时发送占位文本（start 时已标记）
      if (state.pendingImagePlaceholder) {
        state.pendingImagePlaceholder = false;
        let prefix = '';
        if (!state.sentRole) {
          prefix = encodeOpenAIChunk(state.messageId, targetModel, { role: 'assistant' });
          state.sentRole = true;
        ...
        return prefix + encodeOpenAIChunk(state.messageId, targetModel, { content: '[模型输出了一张图片，当前协议不支持透传]' });
      }
      state.blockType = null;
      return '';
    }
