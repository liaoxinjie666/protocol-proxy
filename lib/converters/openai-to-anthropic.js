...
function convertRequest(body, targetModel) {
  ...
  // 转换消息角色和内容（仅在消息自身有 reasoning_content 时才添加 thinking 块）
...
function convertMessage(msg) {
  ...
  const needThinking = msg.role === 'assistant' && msg.reasoning_content;

  if (msg.role === 'assistant' && msg.tool_calls) {
    const content = [];
    if (needThinking) {
      const block = makeThinkingBlock(msg.reasoning_content);
