...
function convertRequest(body, targetModel) {
  ...
  for (const msg of (body.messages || [])) {
    ...
    // 处理数组 content（OpenAI 图片、Anthropic 格式混用）
    if (Array.isArray(msg.content)) {
      const parts = [];
      for (const block of msg.content) {
        if (block.type === 'text' && block.text) parts.push({ text: block.text });
        if (block.type === 'image_url' && block.image_url?.url) {
          const geminiPart = convertImageUrlToGemini(block.image_url.url);
        ...
        if (block.type === 'input_audio' && block.input_audio?.data) {
          ...
          parts.push({ inlineData: { mimeType: `audio/${format}`, data: block.input_audio.data } });
        }
...
function convertImageUrlToGemini(url) {
  if (!url) return null;
  ...
  // Gemini 的 fileData 不支持任意 HTTP URL，base64 图片已覆盖主要使用场景
  return null;
}
