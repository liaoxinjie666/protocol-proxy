...
function convertResponse(openaiBody) {
  ...
  // reasoning_content → thinking 块（放在 text 之前，符合 anthropic 顺序）
  // 仅响应回流方向：thinking 块作为输出内容合法，无需 signature（signature 仅在请求回传时才校验）
  // 跨源降级：只传文本，不涉及 anthropic signature
  if (message.reasoning_content) {
    const rText = typeof message.reasoning_content === 'string'
      ? message.reasoning_content
      : (Array.isArray(message.reasoning_content) ? message.reasoning_content.map(b => b.thinking || b.text || '').join('') : '');
    if (rText && rText.trim()) {
      content.push({ type: 'thinking', thinking: rText });
    }
  }
