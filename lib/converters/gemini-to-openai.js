...
function convertResponse(geminiBody) {
  ...
  for (const part of parts) {
    // 区分 thought（reasoning）和正式回答（content）
  ...
  if (thoughtParts.length > 0) message.reasoning_content = thoughtParts.join('');
  ...
  return {
    convertChunk(chunkText) {
      ...
      for (const line of lines) {
        ...
        // 文本增量：区分 thought（reasoning_content）和正式回答（content）
        const thoughtText = parts.filter(p => p.text && p.thought).map(p => p.text).join('') || '';
        const answerText = parts.filter(p => p.text && !p.thought).map(p => p.text).join('') || '';
        if (thoughtText) {
          output += encodeOpenAIEvent({
            id: '',
            object: 'chat.completion.chunk',
        ...
        if (answerText) {
          output += encodeOpenAIEvent({
            id: '',
            object: 'chat.completion.chunk',
            choices: [{ index: 0, delta: { content: answerText }, finish_reason: null }],
          });
        }
        ...
        if (candidate.finishReason) {
          ...
          // 末块附带 usage（OpenAI 流式约定：最后一个 chunk 带 usage）
          const um = state.usageMetadata;
          const usage = um ? {
