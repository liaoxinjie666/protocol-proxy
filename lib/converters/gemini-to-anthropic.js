...
function createSSEConverter(nameToId = new Map()) {
  const state = { started: false, textBlockStarted: false, textBlockClosed: false, blockIndex: 0, sentFunctionCallPositions: new Set(), nameToId, nameCount: new Map(), buffer: '' };
  ...
  return {
    convertChunk(chunkText) {
      ...
      for (const line of lines) {
        ...
        // 捕获 gemini usageMetadata（每个 chunk 都可能带，最后一次最准确）
        ...
        if (candidate.finishReason) {
          ...
          // 与非流式一致：若本轮发过工具调用，stop_reason 必须是 tool_use，
          // 否则依赖 stop_reason==='tool_use' 判断"该执行工具"的 agentic loop 会错误终止
          const hasToolUse = state.sentFunctionCallPositions.size > 0;
          const stopReason = hasToolUse ? 'tool_use' : mapFinishReason(candidate.finishReason);
          const outTokens = state.usageMetadata?.candidatesTokenCount || 0;
