        ...
        if (isFreeform) {
          ...
          try {
            ...
            if (keys.length > 0 && typeof parsed[keys[0]] === 'string') {
              args = parsed[keys[0]];
              dbg(`[c2r] convertResponse unwrap: keys=${JSON.stringify(keys)} → len=${args.length}`);
            }
          } catch { /* 参数不是 JSON，保持原样 */ }
        }
...
function createSSEConverter(targetModel, freeformTools) {
  ...
  function emitToolCallDone(idx) {
    ...
    // freeform 工具参数解包：从 JSON 包裹 {patch: "..."} 或 {input: "..."} 中提取原始文本
    if (buf.isFreeform) {
      const rawArgs = buf.arguments;
      try {
        const parsed = JSON.parse(buf.arguments);
        const keys = Object.keys(parsed);
        if (keys.length > 0 && typeof parsed[keys[0]] === 'string') {
          buf.arguments = parsed[keys[0]];
          dbg(`[c2r] freeform unwrap: keys=${JSON.stringify(keys)} → len=${buf.arguments.length}`);
        } else {
          dbg(`[c2r] freeform unwrap: JSON parsed but no string value. keys=${JSON.stringify(keys)}`);
        }
      } catch (e) {
        dbg(`[c2r] freeform unwrap: not JSON. error=${e.message}`);
      }
