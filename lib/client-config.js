...
function previewCodex(proxy) {
  ...
  if (currentToml) {
    ...
    // Windows 沙盒只支持 elevated/unelevated，其他无效值会导致客户端初始化失败
    // (Codex 报 "Couldn't set up non-admin sandbox")。有管理员权限用 elevated，否则 unelevated。
    const windows = parseTomlSection(currentToml, 'windows');
    const currentSandbox = windows?.sandbox || null;
    const validSandbox = ['elevated', 'unelevated'];
    if (currentSandbox && !validSandbox.includes(currentSandbox)) {
      const targetSandbox = process.platform === 'win32' ? 'elevated' : 'unelevated';
      changes.push({ field: 'windows.sandbox', old: currentSandbox, new: targetSandbox, note: `${currentSandbox} 不是有效的 Windows 沙盒值，会导致客户端启动失败，改为 ${targetSandbox}` });
    }
  } else {
...
function writeCodex(proxy) {
  ...
  if (toml) {
    ...
    // Windows 沙盒只支持 elevated/unelevated，无效值（如 permissive）会让客户端启动失败
    // (Codex 报 "Couldn't set up non-admin sandbox")。有管理员权限默认 elevated。
    const windows = parseTomlSection(toml, 'windows');
    const validSandbox = ['elevated', 'unelevated'];
    if (windows && windows.sandbox && !validSandbox.includes(windows.sandbox)) {
      const targetSandbox = process.platform === 'win32' ? 'elevated' : 'unelevated';
      toml = updateTomlSection(toml, 'windows', { sandbox: targetSandbox });
    }
  } else {
