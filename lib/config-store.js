...
function diffObjects(oldObj, newObj, prefix) {
  ...
  for (const key of allKeys) {
    ...
    if (!inOld && inNew) {
      result.added.push({ path, value: newObj[key] });
    } else if (inOld && !inNew) {
      result.removed.push({ path, oldValue: oldObj[key] });
    } else {
      const oldVal = oldObj[key];
      const newVal = newObj[key];
      if (Array.isArray(oldVal) || Array.isArray(newVal)) {
        // 数组用索引逐一对比
        const maxLen = Math.max(oldVal?.length || 0, newVal?.length || 0);
        for (let i = 0; i < maxLen; i++) {
          const itemPath = `${path}[${i}]`;
          if (i >= (oldVal?.length || 0)) {
            result.added.push({ path: itemPath, value: newVal[i] });
          } else if (i >= (newVal?.length || 0)) {
            result.removed.push({ path: itemPath, oldValue: oldVal[i] });
          } else if (typeof oldVal[i] === 'object' && oldVal[i] !== null && typeof newVal[i] === 'object' && newVal[i] !== null) {
            const subDiff = diffObjects(oldVal[i], newVal[i], itemPath);
            ...
            result.changed.push({ path: itemPath, oldValue: oldVal[i], newValue: newVal[i] });
          }
        }
      } else if (typeof oldVal === 'object' && oldVal !== null && typeof newVal === 'object' && newVal !== null) {
        const subDiff = diffObjects(oldVal, newVal, path);
        result.added.push(...subDiff.added);
        result.removed.push(...subDiff.removed);
        result.changed.push(...subDiff.changed);
      } else if (JSON.stringify(oldVal) !== JSON.stringify(newVal)) {
        result.changed.push({ path, oldValue: oldVal, newValue: newVal });
      }
    }
  }
  return result;
