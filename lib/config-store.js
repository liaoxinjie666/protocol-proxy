...
function normalizeModels(models) {
  ...
  const seen = new Set();
  const VALID_EFFORT = ['', 'high', 'max'];
  return models.map(m => {
    if (typeof m === 'string') return { name: m.trim(), contextLength: 0, thinkingEffort: '' };
    if (m && typeof m === 'object' && typeof m.name === 'string') {
      const effort = VALID_EFFORT.includes(m.thinkingEffort) ? m.thinkingEffort : '';
      return { name: m.name.trim(), contextLength: Math.max(0, parseInt(m.contextLength) || 0), thinkingEffort: effort };
    }
    return null;
  }).filter(m => m && m.name && !seen.has(m.name) && seen.add(m.name));
}
...
function normalizeProvider(provider) {
  ...
  const capabilities = Array.isArray(provider.capabilities)
    ? Array.from(new Set(provider.capabilities.filter(c => typeof c === 'string').map(c => c.trim()).filter(Boolean)))
    : [];
  return {
    ...provider,
...
function getMultimodalServices() {
  return loadConfig().multimodalServices || [];
}

function getMultimodalServiceById(id) {
  return getMultimodalServices().find(s => s.id === id);
}

function addMultimodalService(service) {
  const config = loadConfig();
  config.multimodalServices = config.multimodalServices || [];
  service = { ...service, id: service.id || crypto.randomUUID(), enabled: service.enabled !== false, createdAt: Date.now() };
  config.multimodalServices.push(service);
  saveConfig(config);
  return service;
}

function updateMultimodalService(id, updates) {
  const config = loadConfig();
  const idx = (config.multimodalServices || []).findIndex(s => s.id === id);
  if (idx === -1) return null;
  config.multimodalServices[idx] = { ...config.multimodalServices[idx], ...updates, id };
  saveConfig(config);
  return config.multimodalServices[idx];
}

function removeMultimodalService(id) {
  const config = loadConfig();
  const idx = (config.multimodalServices || []).findIndex(s => s.id === id);
  if (idx === -1) return null;
  const removed = config.multimodalServices.splice(idx, 1)[0];
  saveConfig(config);
  return removed;
}
...
module.exports = {
  ...
  updateProvider,
  removeProvider,
  getMultimodalServices,
  getMultimodalServiceById,
  addMultimodalService,
  updateMultimodalService,
  removeMultimodalService,
  getProxies,
  getProxyById,
  addProxy,
  updateProxy,
