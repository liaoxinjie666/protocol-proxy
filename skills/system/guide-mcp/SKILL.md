...
参数：-y open-websearch
环境变量：MODE=stdio
环境变量：PLAYWRIGHT_HEADLESS=true
```
**用途**：搜索网页内容、抓取文章、获取实时信息
**特点**：无需 API Key，完全免费，支持 Bing/Baidu/DuckDuckGo 等多引擎
**注意**：必须设置 `MODE=stdio`，否则默认 `MODE=both` 会同时启动 HTTP 服务器，端口冲突会导致整个进程崩溃（包括 STDIO 传输）
...
4. **端口被占用**
   - 某些 MCP 服务（如 open-websearch）默认同时启动 HTTP 服务器，端口冲突会导致进程崩溃
   - 解决：设置环境变量 `MODE=stdio` 禁用 HTTP 服务器（推荐），或设置 `PORT=其他端口`
...
3. **端口冲突**
   - 某些 MCP 服务（如 open-websearch）默认同时启动 HTTP 服务器，端口冲突会导致进程崩溃
   - 推荐方案：设置环境变量 `MODE=stdio` 禁用 HTTP 服务器
   - 备选方案：设置 `PORT=其他端口` 避免冲突
